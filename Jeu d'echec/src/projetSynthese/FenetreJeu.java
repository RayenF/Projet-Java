				...
				for (int i = 0; i < 8; i++)
					...
					if((e.getCase(colonneClic, ligneClic).getPiece() != null | pieceTampon != null) )
					{
						...
						else
						{
							...
							{
								//je créé un jLabel avec l'icône de la pi?ce manger
								...
								/* je vérifie si la pi?ce manger est un roi, si oui le jeu est terminé et L'utilisateurs 
								peut choisir si il veut continuer a jouer ou non*/
								if(e.getCase(colonneClic, ligneClic).getPiece() instanceof Roi)
								{
									if(JOptionPane.showConfirmDialog(null, "Le roi a ÈtÈ dÈfait, nous nous rendons\n", "Mince !", JOptionPane.YES_NO_OPTION) == 0){
										RAZ();
